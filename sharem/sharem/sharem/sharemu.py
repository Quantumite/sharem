...
from .modules import (
    initMods,
    saveDLLAddsToFile,
    allocateWinStructs32,
    allocateWinStructs64,
)
...
from sharem.sharem.DLLs.hookAPIs import (
    CustomWinAPIs,
    CustomWinSysCalls,
    stackCleanup,
    read_string,
...
class Coverage:
            ...
            try:
                ...
                if em.showCCDebugInfo:
                    print(
                        ...
                        + "\t[*] "
                        + constants.WHITE
                        + "Code coverage: Could not capture memory pointed to by esp - memory not valid: "
                        + constants.RESET,
                        hex(esp),
                    )
            try:
                ...
                if em.showCCDebugInfo:
                    print(
                        ...
                        + "\t[*] "
                        + constants.WHITE
                        + "Code coverage: Could not capture memory pointed to by ebp - memory not valid: "
                        + +constants.RESET,
                        hex(ebp),
                    )
        if em.arch == 64:
            ...
            try:
                ...
                if em.showCCDebugInfo:
                    print(
                        constants.constants.RED
                        + "\t[*] "
                        + constants.WHITE
                        + "Code coverage: Could not capture memory pointed to by rsp - memory not valid: "
                        + +constants.RESET,
                        hex(rsp),
                    )
            try:
                ...
                if em.showCCDebugInfo:
                    print(
                        ...
                        + "\t[*] "
                        + constants.WHITE
                        + "Code coverage: Could not capture memory pointed to by rbp - memory not valid: "
                        + +constants.RESET,
                        hex(rbp),
                    )
    ...
    def dump_saved_info(self, uc):
        ...
        if em.arch == 32:
            ...
            try:
                ...
                print(
                    "\tComplete code coverage: restoring memory pointed to by esp,",
                    hex(self.regs["esp"]),
                    ", failed for coverage object",
                    self.coverage_num,
                    ".",
                )
            try:
                ...
                print(
                    "\tComplete code coverage: restoring memory pointed to by ebp,",
                    hex(self.regs["ebp"]),
                    ", failed for coverage object",
                    self.coverage_num,
            ...
            try:
                ...
                print(
                    "\tComplete code coverage: restoring memory pointed to by rsp,",
                    hex(self.regs["rsp"]),
                    ", failed for coverage object",
                    self.coverage_num,
                    ".",
            ...
            try:
                ...
                print(
                    "\tComplete code coverage: restoring memory pointed to by rbp,",
                    hex(self.regs["rbp"]),
                    ", failed for coverage object",
                    self.coverage_num,
                    ". This may be correct behavior.",
    ...
    def delete(self, index):
        if em.showCCDebugInfo:
            print(
                constants.YELLOW
                + "\t[*] Deleting code coverage object:"
                + constants.RESET,
                coverage_objects[index].coverage_num,
                constants.GREEN + "   Address:",
                constants.WHITE + hex(coverage_objects[index].address),
            )
    ...
    def giveAddress(self, address):
        print(
            constants.CYAN + "Coverage - adding address",
            hex(address),
            "num" + +constants.RESET,
            self.coverage_num,
        )
...
codeLen = 0
with open(
    os.path.join(os.path.dirname(__file__), "WinSysCalls.json"), "r"
) as syscall_file:
    syscall_dict = json.load(syscall_file)

with open(
    os.path.join(os.path.dirname(__file__), "skipAddressesCCC.json"), "r"
) as jsonCCC:
...
foundDLLAddresses32 = os.path.join(
    os.path.dirname(__file__), "foundDLLAddresses32.json"
)
foundDLLAddresses64 = os.path.join(
    os.path.dirname(__file__), "foundDLLAddresses64.json"
)
...
def catch_windows_api(uc, fRaw, addr, ret, size, funcAddress):
    ...
    try:
        funcInfo, cleanBytes = getattr(WinAPI, funcName)(
            uc, eip, esp, export_dict, addr, em
        )
...
def hook_mem_access(uc, access, address, size, value, user_data):
    if access == UC_MEM_WRITE:  # noqa: F405
        ...
        if (
            address < MOD_LOW
            or address > MOD_HIGH
            and (address < STACK_ADDR - 0x5000 or address > STACK_ADDR + 0x5000)
            and address != ENTRY_ADDR
            and address != EXTRA_ADDR
        ):
...
def hook_code(uc, address, size, user_data):
    ...
    if programCounter > em.maxCounter and em.maxCounter > 0:
        print(
            ...
            + "\t[*] "
            + constants.RESET
            + " Exiting emulation because max counter of "
            + constants.GREEN
            + str(em.maxCounter)
            + constants.RESET
            + " reached.\n"
    ...
    if jmpFlag != "":
            ...
            if jumpAddr not in coverageAdds:
                ...
                if em.showCCDebugInfo:
                    print(
                        ...
                        + "\t[*] "
                        + constants.CYAN
                        + "Creating code coverage object:"
                        + constants.RESET,
                        cvg1.coverage_num,
                        constants.GREEN + "   Address:" + constants.RESET,
                        hex(jumpAddr),
                    )
        ...
        "call" in mnemonic and em.includeCallInCC and em.codeCoverage
    ):  # we are adding CALL as well.
        if address + size not in coverageAdds and address + size not in skipForCoverage:
            ...
            if em.showCCDebugInfo:
                print(
                    ...
                    + "\t[*] "
                    + constants.CYAN
                    + "Code Coverage CALL - adding address"
                    + constants.RESET,
                    hex(address + size),
                    constants.CYAN + "- coverage object:" + constants.RESET,
                    cvg2.coverage_num,
                )
    elif (
        "jmp" in mnemonic and em.codeCoverage and em.includeJmpInCC
    ):  # we are adding CALL as well.
        if address + size not in coverageAdds and address + size not in skipForCoverage:
            ...
            if em.showCCDebugInfo:
                print(
                    ...
                    + "\t[*] "
                    + constants.CYAN
                    + "Code Coverage JMP - adding address"
                    + constants.RESET,
                    hex(address + size),
                    constants.CYAN + "- coverage object:" + constants.RESET,
                    cvg2.coverage_num,
                ...
                if em.showCCDebugInfo:
                    if stopProcessCC:
                        print(
                            ...
                            + "\t[*]"
                            + constants.RESET
                            + " Complete code coverage: already traversed "
                            + hex(address)
                            + constants.RED
                            + " -  stopping."
                            + constants.RESET
                        ...
                        print(
                            ...
                            + hex(address)
                            + constants.RED
                            + " -  stopping after next instruction."
                            + constants.RESET
                        )
...
def hook_syscallBackup(uc, eip, esp, funcAddress, funcName, callLoc, syscallID):
    try:
        for dictionary_candidate in (
            (dict_kernel32, "kernel32"),
            (dict_ntdll, "ntdll"),
            (dict_user32, "user32"),
        ):  # noqa: F405
            try:
...
def findArtifacts():
    ...
    for p in paramValues:
        ...
        art.file_artifacts += re.findall(
            Regex.find_totalFilesBeginning, str(p), re.IGNORECASE
        )

        # -------------------------------------------
        #       Finding Command line
        # -------------------------------------------
        art.commandLine_artifacts += re.findall(
            Regex.total_commandLineArguments, str(p), re.IGNORECASE
        )
        ...
        #       Finding Registry
        # -------------------------------------------
        art.registry_artifacts += re.findall(
            Regex.total_Registry, str(p), re.IGNORECASE
        )
...
def restartEmu(mu, mode, code):
    ...
    try:
        if mode == UC_MODE_32:
            ...
            if verbose:
                outFile.write(
                    "***** Complete code coverage: restarting emulation of x86 shellcode at "
                    + hex(startLoc)
                    + ".\n"
                )
            if em.timeless_debugging_stack:
            ...
            if verbose:
                outFile.write(
                    "\n***** Complete code coverage: restarting emulation of x86_64 shellcode at "
                    + hex(startLoc)
                    + ".\n"
                )
            if em.timeless_debugging_stack:
