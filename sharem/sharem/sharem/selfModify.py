...
def disherePEB_decrypt(
    ...
    for i in cs.disasm(CODED3, address):
        ...
        loadLDR = re.match(
            "^((mov)|(add)|(xor)|(or)|(adc)|(xchg)) (e?((ax)|(bx)|(cx)|(dx)|(di)|(si)|(bp))|((a|b|c|d)(h|l))), ?(d?word ptr ?(ds:)?\[(e?((ax)|(bx)|(cx)|(dx)|(di)|(si)|(bp))|((a|b|c|d)(h|l))) ?\+ ?(0xc)\])",
            val,
            re.IGNORECASE,
        )
...
def findAllPebSequences_decode(
    mode, inputBytes
):  ################## AUSTIN ######################
    # global rawHex
    ...
    for match in (
        ...
        ans = get_PEB_walk_start_decode(
            mode, 19, match, "noSec", inputBytes
...
def specialEncoder_old(*args):  # args = variable # of inputs
    ...
    print(
        "spread across " + str(cores) + " cores: ",
        ((((totalPerm * numSeconds) / 60) / 60) / 24) / cores,
        "days",
    )
        ...
        for each in list(itertools.permutations(mylist2)):
            ...
            while w < maxValuW:
                ...
                while u < maxValuU:
                    ...
                    bytesStr = bytes(
                        encodeBytes2
                    )  ####  USE this global to get results from function
...
def specialEncoder2(Max, *args):
    ...
    totalPerm2 = (
        ...
        * maxValuR
        * maxValuV
        * len(list(itertools.permutations(mylist2)))
    )
    ...
    if __name__ == "__main__":
        ...
        processList.append(
            multiprocessing.Process(
                target=doStuff_threads,
                args=(procnum, return_dict, each, a, b, c, sample[0:index]),
            )
        )
        procnum = 2
        processList.append(
            multiprocessing.Process(
                target=doStuff_threads,
                args=(procnum, return_dict, each, a, b, c, sample[index:]),
            )
        )
            ...
            processList.append(
                multiprocessing.Process(
                    ...
                    args=(
                        ...
                        b,
                        c,
                        sample[startChunk : startChunk + rSize],
                    ),
                )
            )
            startChunk += rSize
            ...
            processList.append(
                multiprocessing.Process(
                    ...
                    args=(
                        ...
                        b,
                        c,
                        sample[startChunk : startChunk + p_rSize],
                    ),
                )
            )
            startChunk += p_rSize
...
def austinDecode(
    ...
    if len(mylist2) == 1:
        ...
        totalPerm = (
            aLimit * bLimit * cLimit * dLimit * eLimit * (nPr(numItems, numItems - 1))
        )
    ...
    if single:
        ...
        out = runProcs(
            encodeBytes4, sample, numThreads, version, findAll=findAll
        )[
            0
        ]  # the first element returned is the actual output, do not need to worry about the flag
            ...
            for a in numVals:
                while eachInd < eachLen:
                    ...
                    if curPerm > listLimit:
                        ...
                        rpOut = runProcs(
                            encodeBytes4, sample, numThreads, 1, findAll=findAll
                        )
                        out = out + rpOut[0]
                        matched = rpOut[1]
                ...
                for b in numVals:
                    ...
                    while eachInd < eachLen:
                        ...
                        if curPerm > listLimit:
                            ...
                            # print("RUNNING PROCS EARLY, TOTALRUNS = ",totalRuns)
                            rpOut = runProcs(
                                encodeBytes4, sample, numThreads, 2, findAll=findAll
                            )
                            out = out + rpOut[0]
                            matched = rpOut[1]
                            # print("CAME BACK FROM RUNPROCS, rpOut = ", rpOut)
                    ...
                    for c in numVals:
                        while eachInd < eachLen:
                            ...
                            if curPerm > listLimit:
                                ...
                                # print("RUNNING PROCS EARLY, TOTALRUNS = ",totalRuns)
                                rpOut = runProcs(
                                    encodeBytes4, sample, numThreads, findAll=findAll
                                )
                                out = out + rpOut[0]
                                matched = rpOut[1]
                                # print("CAME BACK FROM RUNPROCS, rpOut = ", rpOut)
                            ...
                            for e in numVals:
                                while eachInd < eachLen:
                                    if matched == 1:
                                        ...
                                        return out, early, startVals
                                    encodeBytes4.append(
                                        (a, b, c, d, e, permutations[eachInd])
                                    )
                                    eachInd += 1
                                    curPerm += 1
                                    if curPerm > listLimit:
                    ...
                    for cNum in numVals:
                        while eachInd < eachLen:
                            if matched == 1:
                                ...
                                return out, early, startVals
                            encodeBytes4.append(
                                (aNum, bNum, cNum, permutations[eachInd])
                            )
                            eachInd += 1
                            curPerm += 1
                            if curPerm > listLimit:
                        ...
                        for dNum in numVals:
                            while eachInd < eachLen:
                                if matched == 1:
                                    ...
                                    return out, early, startVals
                                encodeBytes4.append(
                                    (aNum, bNum, cNum, dNum, permutations[eachInd])
                                )
                                eachInd += 1
                                curPerm += 1
                                if curPerm > listLimit:
...
def doDistr(
    ...
    cluster = dispy.JobCluster(
        austinDecodeDistributed_new,
        nodes=nodeIPs,
        depends=[
            findAllPebSequences_decode,
            nPr,
            findObfusMethod,
            tempMax,
            runProcsDistr,
    ...
    for i in range(numNodes):
        ...
        job = cluster.submit(
            decodeOps, sample, numNodes, i, mode, starts, order, findAll, successPoints
        )
        jobs.append(job)
            ...
            for job in jobs:
                ...
                if job.result is not None:
                    ...
                    if outputs is not None:
                        finalOutput.append(outputs[0])
                    print(
                        "(%s) executed job %s at %s"
                        % (job.ip_addr, job.id, job.start_time)
                    )
    ...
    if len(mylist2) == 1:
        ...
        if totalRuns != totalPerm:
            for nodeID in range(nodes):
                ...
                print(
                    "BOTTOM BLOCK LOW = "
                    + str(block_low(nodeID, nodes, len(encodeBytes4)))
                )
                print(
                    "BOTTOM BLOCK HIGH = "
                    + str(block_high(nodeID, nodes, len(encodeBytes4)))
                )
                out = out + runProcs(
                    encodeBytes4[
                        block_low(nodeID, nodes, len(encodeBytes4)) : block_high(
                            nodeID, nodes, len(encodeBytes4)
                        )
                    ],
                    sample,
                    numThreads,
                    ...
                    while d < dLimit:
                        while eachInd < eachLen:
                            ...
                            if curPerm > listLimit:
                                print("RUNNING PROCS")
                                out = out + runProcs(
                                    encodeBytes4, sample, numThreads, 4
                                )
                                totalRuns += numThreads
                                curPerm = 0
                                encodeBytes4 = []
                        ...
                        while e < eLimit:
                            while eachInd < eachLen:
                                ...
                                if curPerm > listLimit:
                                    print("RUNNING PROCS")
                                    out = out + runProcs(
                                        encodeBytes4, sample, numThreads, 5
                                    )
                                    totalRuns += numThreads
                                    curPerm = 0
                                    encodeBytes4 = []
...
def austinDecodeDistributed_new(
    decodeOps,
    sample,
    nodes,
    nodeID,
    ...
    try:
        ...
        if len(mylist2) == 1:
            ...
            totalPerm = (
                aLimit * bLimit * cLimit * dLimit * (nPr(numItems, numItems - 1))
            )
        elif len(mylist2) == 5:
            totalPerm = (
                aLimit
        ...
        if len(mylist2) == 1:
                    ...
                    if curPerm > listLimit:
                        ...
                        rpOut = runProcsDistr(
                            encodeBytes4[
                                block_low(
                                    nodeID, nodes, len(encodeBytes4)
                                ) : block_high(nodeID, nodes, len(encodeBytes4))
                            ],
                            sample,
                            numThreads,
            ...
            if totalRuns < totalPerm:
                ...
                printOut += (
                    "BOTTOM ENCODEBYTES LEN = " + str(len(encodeBytes4))
                ) + "\n"
    ...
    for rank in range(numThreads):
        ...
        processList.append(
            multiprocessing.Process(
                target=p2Encode,
                args=(
                    block_low(rank, numThreads, argsLen),
                    block_high(rank, numThreads, argsLen),
                    encodeBytes4,
                    sample,
                    rank,
                    queue,
...
def runProcsDistr(
    encodeBytes4, sample, numThreads, version=3, findAll=False, successPoints=3
):
    from distrFunc import (
        p2EncodeDistr,
        doStuffP2Distr,
...
def p2Encode(
    low, high, encodeBytes4, sample, rank, queue, version, endFlag, findAll=False
):
    outs = []
    # if(version == 3):
    ...
    if len(mylist2) == 3:
        ...
        if __name__ == "__main__":
            ...
            for procnum in range(len(perms)):
                ...
                processList.append(
                    multiprocessing.Process(
                        target=permHelp1, args=(queue, (each,), aLimit, bLimit, cLimit)
                    )
                )
...
def permHelp1(queue, each, aLimit, bLimit, cLimit):
    ...
    encodeBytes4 = [
        doStuff_testP(each, a, b, c, sample)
        for a in range(aLimit)
        for b in range(bLimit)
        for c in range(cLimit)
    ]
            ...
            while procnum < len(perms):
                ...
                while (procnum < len(perms)) and (loopThreads < numThreads):
                    ...
                    processList.append(
                        multiprocessing.Process(
                            target=threadEncode,
                            args=(queue, (each,), aLimit, bLimit, cLimit),
                        )
                    )
                    procnum = procnum + 1
...
def threadEncode(queue, each, aLimit, bLimit, cLimit):
    ...
    # encodeBytes4 = permHelp2(each,sample)
    encodeBytes4 = [
        doStuffParallel(each, a, b, c, sample)
        for a in range(aLimit)
        for b in range(bLimit)
        for c in range(cLimit)
    ]
...
def specialEncoder5q(*args):
    if __name__ == "__main__":
        ...
        if len(mylist2) == 3:
            encodeBytes4 = [
                doStuff_testQ(each, a, b, c, sample)
                for a in range(aLimit)
                for each in (list(itertools.permutations(mylist2)))
                for b in range(bLimit)
                for c in range(cLimit)
...
def specialEncoder5_test(*args):
    if __name__ == "__main__":
        ...
        encodeBytes4 = [
            pool.apply(
                testPool5, args=(mylist2, each, 0, 0, 0, sample, aLimit, bLimit, cLimit)
            )
        ]
        stop = timeit.default_timer()
        print("Total time: " + str(stop - start))
...
def testPool5(mylist2, each, a, b, c, sample, aLimit, bLimit, cLimit):
    if len(mylist2) == 3:
        encodeBytes4 = [
            doStuff_test(each, a, b, c, sample)
            for a in range(aLimit)
            for each in (list(itertools.permutations(mylist2)))
            for b in range(bLimit)
            for c in range(cLimit)
...
def specialEncoder6(myInput):
    ...
    if len(mylist2) == 3:
        outputs_async = [
            pool.map_async(test101, (each, a, b, c, sample))
            for a in range(aLimit)
            for each in (list(itertools.permutations(mylist2)))
            for b in range(bLimit)
            for c in range(cLimit)
