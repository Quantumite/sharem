...
class Configuration(metaclass=Singleton):
    def __init__(
        self,
        cfgFile=os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.cfg"),
    ):
        ...
        self.display_non_traversed_code_and_data_as_cyan_in_disassembly_if_CCC_used = (
            True
        )
    ...
    def changeConf(self, *args):
        ...
        self.comments = self.config.getboolean(
            "CONFIG", "include_comments_in_this_file"
        )
        ...
        for key, val in self.args.items():
            ...
            for x in sharem_emuSimValues:
                if key in x:
                    self.config["SHAREM EMULATION SIMULATED VALUES"][str(key)] = str(
                        val
                    )
    ...
    def decryptConf(self, conr):
        ...
        try:
            self.decrypt_dec_operation_type = ast.literal_eval(
                self.decrypt_dec_operation_type
            )
        except:
            print(
                constants.YELLOW + "The value of",
                constants.RED + self.decrypt_dec_operation_type,
                constants.YELLOW + "is not correct or malformed!!" + constants.RED,
            )
            sys.exit()
        ...
        try:
            self.decrypt_stub_entry_point = int(
                conr["SHAREM DECRYPT"]["stub_entry_point"]
            )
        except:
            self.decrypt_stub_entry_point = int(
                conr["SHAREM DECRYPT"]["stub_entry_point"], 16
            )
    ...
    def searchConf(self, conr):
        ...
        self.search_max_callpop_distance = int(
            conr["SHAREM SEARCH"]["max_callpop_distance"]
        )
        ...
        try:
            self.search_max_bytes_forward = int(
                conr["SHAREM SEARCH"]["max_bytes_forward"]
            )
        except:
            self.search_max_bytes_forward = int(
                conr["SHAREM SEARCH"]["max_bytes_forward"], 16
            )

        try:
            self.search_max_bytes_backward = int(
                conr["SHAREM SEARCH"]["max_lines_backward"]
            )
        except:
            self.search_max_bytes_backward = int(
                conr["SHAREM SEARCH"]["max_lines_backward"], 16
            )

        try:
            self.search_max_lines_forward = int(
                conr["SHAREM SEARCH"]["max_lines_forward"]
            )
        except:
            self.search_max_lines_forward = int(
                conr["SHAREM SEARCH"]["max_lines_forward"], 16
            )

        try:
            self.search_max_lines_backward = int(
                conr["SHAREM SEARCH"]["max_lines_backward"]
            )
        except:
            self.search_max_lines_backward = int(
    ...
    def disassemblyConf(self, conr):
        ...
        self.dissassembly_enable_assembly_comments = conr.getboolean(
            "SHAREM DISASSEMBLY", "enable_assembly_comments"
        )
        self.dissassembly_enable_assembly_ascii = conr.getboolean(
            "SHAREM DISASSEMBLY", "enable_assembly_ascii"
        )
        self.dissassembly_enable_find_strings = conr.getboolean(
            "SHAREM DISASSEMBLY", "enable_find_strings"
        )
        self.dissassembly_ignore_dis_discovery = conr.getboolean(
            "SHAREM DISASSEMBLY", "ignore_dis_discovery"
        )
        self.dissassembly_max_disassembly_operands = int(
            conr["SHAREM DISASSEMBLY"]["max_disassembly_operands"]
        )
        self.dissassembly_binary_to_string = int(
            conr["SHAREM DISASSEMBLY"]["binary_to_string"]
        )
        self.dissassembly_shellcode_size_limit = int(
            conr["SHAREM DISASSEMBLY"]["shellcode_size_limit"]
        )
        self.dissassembly_show_disassembly_offsets = conr.getboolean(
            "SHAREM DISASSEMBLY", "show_disassembly_offsets"
        )
        self.dissassembly_show_disassembly_opcodes = conr.getboolean(
            "SHAREM DISASSEMBLY", "show_disassembly_opcodes"
        )
        self.dissassembly_show_disassembly_labels = conr.getboolean(
    ...
    def emulationConf(self, conr):
        self.emulation_print_emulation_result = conr.getboolean(
            "SHAREM EMULATION", "print_emulation_result"
        )
        self.emulation_verbose_mode = conr.getboolean(
            "SHAREM EMULATION", "emulation_verbose_mode"
        )
        self.emulation_multiline = conr.getboolean(
            "SHAREM EMULATION", "emulation_multiline"
        )
        self.emulation_max_num_of_instr = int(
            conr["SHAREM EMULATION"]["max_num_of_instr"]
        )
        self.emulation_iterations_before_break = int(
            conr["SHAREM EMULATION"]["iterations_before_break"]
        )
        self.emulation_break_infinite_loops = conr.getboolean(
            "SHAREM EMULATION", "break_infinite_loops"
        )
        self.emulation_verbose_mode = conr.getboolean(
            "SHAREM EMULATION", "timeless_debugging"
        )
        self.emulation_complete_code_coverage = conr.getboolean(
            "SHAREM EMULATION", "complete_code_coverage"
        )

        self.emulation_windows_version = conr["SHAREM EMULATION"]["windows_version"]
        self.emulation_windows_release_osbuild = conr["SHAREM EMULATION"][
            "windows_release_osbuild"
        ]
        self.emulation_windows_syscall_code = conr["SHAREM EMULATION"][
            "windows_syscall_code"
        ]

        self.timeless_debugging_stack = conr.getboolean(
    ...
    def emulationSimValueConf(self, conr):
        self.simulatedValues_current_user = conr["SHAREM EMULATION SIMULATED VALUES"][
            "current_user"
        ]
        self.simulatedValues_computer_name = conr["SHAREM EMULATION SIMULATED VALUES"][
            "computer_name"
        ]
        self.simulatedValues_temp_file_prefix = conr[
            "SHAREM EMULATION SIMULATED VALUES"
        ]["temp_file_prefix"]
        self.simulatedValues_default_registry_value = conr[
            "SHAREM EMULATION SIMULATED VALUES"
        ]["default_registry_value"]
        self.simulatedValues_computer_ip_address = conr[
            "SHAREM EMULATION SIMULATED VALUES"
        ]["computer_ip_address"]
        self.simulatedValues_timezone = conr["SHAREM EMULATION SIMULATED VALUES"][
            "timezone"
        ]
        self.simulatedValues_system_time_since_epoch = int(
            conr["SHAREM EMULATION SIMULATED VALUES"]["system_time_since_epoch"]
        )
        self.simulatedValues_system_uptime_minutes = int(
            conr["SHAREM EMULATION SIMULATED VALUES"]["system_uptime_minutes"]
        )
        self.simulatedValues_clipboard_data = conr["SHAREM EMULATION SIMULATED VALUES"][
            "clipboard_data"
        ]
        self.simulatedValues_users = ast.literal_eval(
            conr["SHAREM EMULATION SIMULATED VALUES"]["users"]
        )
        self.simulatedValues_drive_letter = conr["SHAREM EMULATION SIMULATED VALUES"][
            "drive_letter"
        ]
        self.simulatedValues_start_directory = conr[
            "SHAREM EMULATION SIMULATED VALUES"
        ]["start_directory"]
        self.simulatedValues_download_files = conr["SHAREM EMULATION SIMULATED VALUES"][
            "file_download"
        ]

    def stringsConf(self, conr):
        self.strings_push_stack_strings = conr.getboolean(
            "SHAREM STRINGS", "push_stack_strings"
        )
        self.strings_ascii_strings = conr.getboolean("SHAREM STRINGS", "ascii_strings")
        self.strings_wide_char_strings = conr.getboolean(
            "SHAREM STRINGS", "wide_char_strings"
        )
        self.strings_minimum_str_length = int(
            conr["SHAREM STRINGS"]["minimum_str_length"]
        )
    ...
    def patternConf(self, conr):
        ...
        self.pattern_lang_code_pattern = int(
            conr["SHAREM PATTERNS"]["lang_code_pattern"]
        )
        self.pattern_dotted_word_pattern = int(
            conr["SHAREM PATTERNS"]["dotted_word_pattern"]
        )
