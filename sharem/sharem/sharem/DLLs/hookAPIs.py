...
class CustomWinAPIs:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "\tError: The shellcode tried to load a DLL that isn't handled by this tool: ",
                        name,
                    )
                    retVal = 0
    ...
    def WriteFile(
        ...
        if handle not in HandlesDict:
            ...
            handle.name = SimFileSystem.detectDuplicateFileHandles(
                SimFileSystem.currentDir, handle
            )
    ...
    def BroadcastSystemMessageA(
        ...
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        flagsReverseLookUp = {
            0x00000080: "BSF_ALLOWSFW",
            0x00000004: "BSF_FLUSHDISK",
            0x00000020: "BSF_FORCEIFHUNG",
            0x00000002: "BSF_IGNORECURRENTTASK",
            0x00000008: "BSF_NOHANG",
            0x00000040: "BSF_NOTIMEOUTIFNOTHUNG",
            0x00000010: "BSF_POSTMESSAGE",
            0x00000001: "BSF_QUERY",
            0x00000100: "BSF_SENDNOTIFYMESSAGE",
        }
        lpInfoReverseLookUp = {
            0x00000000: "BSM_ALLCOMPONENTS",
            0x00000010: "BSM_ALLDESKTOPS",
    ...
    def CryptDecrypt(
        ...
        dwFlagsReverseLookUp = {
            64: "CRYPT_OAEP",
            32: "CRYPT_DECRYPT_RSA_NO_PADDING_CHECK",
        }
    ...
    def xxGetLogicalProcessorInformation(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
    ...
    def xxGetLogicalProcessorInformationEx(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        pTypes = [
            "LOGICAL_PROCESSOR_RELATIONSHIP",
            "PSYSYEM_LOGICAL_PROCESSOR_INFORMATION_EX",
            "PDWORD",
        ]
    ...
    def xxExpandEnvironemtnStringsA(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
    ...
    def CreateToolhelp32Snapshot(
        ...
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        dwFlagsReverseLookUp = {
            2147483648: "TH32CS_INHERIT",
            15: "TH32CS_SNAPALL",
            1: "TH32CS_SNAPHEAPLIST",
            8: "TH32CS_SNAPMODULE",
            16: "TH32CS_SNAPMODULE32",
    ...
    def Process32First(
        ...
        if pVals[0] in HandlesDict:
            ...
            SnapShot = System_SnapShot(True, True)
            handle = Handle(
                HandleType.CreateToolhelp32Snapshot, data=SnapShot, handleValue=pVals[0]
            )
    ...
    def Process32Next(
        ...
        try:
            ...
            handle.data.processOffset = processList[
                processList.index(handle.data.processOffset) + 1
            ]
        except:
    ...
    def Thread32Next(
        ...
        try:
            ...
            handle.data.threadOffset = threadList[
                threadList.index(handle.data.threadOffset) + 1
            ]
        except:
    ...
    def Toolhelp32ReadProcessMemory2(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):  # Needs to be Redone
    ...
    def URLDownloadToFileA(
        ...
        origin = pVals[1]
        destination = pVals[2]
        filePath, fileName, datahash, statusCode = SimFileSystem.internetDownload(
            origin, destination
        )
    ...
    def InternetOpenA(
        ...
        dwFlagsReverseLookUp = {
            268435456: "INTERNET_FLAG_ASYNC",
            16777216: "INTERNET_FLAG_FROM_CACHE",
        }
    ...
    def InternetConnectA(
        ...
        nServerPortReverseLookUp = {
            0: "INTERNET_INVALID_PORT_NUMBER",
            33: "INTERNET_DEFAULT_FTP_PORT",
            112: "INTERNET_DEFAULT_GOPHER_PORT",
            128: "INTERNET_DEFAULT_HTTP_PORT",
            1091: "INTERNET_DEFAULT_HTTPS_PORT",
            4224: "INTERNET_DEFAULT_SOCKS_PORT",
        }
        dwServiceReverseLookUp = {
            1: "INTERNET_SERVICE_FTP",
            2: "INTERNET_SERVICE_GOPHER",
            3: "INTERNET_SERVICE_HTTP",
        }
        dwFlagsReverseLookUp = {
            536870912: "INTERNET_FLAG_EXISTING_CONNECT",
            134217728: "INTERNET_FLAG_PASSIVE",
            1: "WININET_API_FLAG_ASYNC",
            4: "WININET_API_FLAG_SYNC",
        }
    ...
    def RegCreateKeyExA(
        ...
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        dwOptionsReverseLookUp = {
            4: "REG_OPTION_BACKUP_RESTORE",
            2: "REG_OPTION_CREATE_LINK",
            0: "REG_OPTION_NON_VOLATILE",
            1: "REG_OPTION_VOLATILE",
        }
        lpdwDispostitionReverseLookUp = {
    ...
    def RegGetValueA(
        ...
        dwFlagsReverseLookUp = {
            ...
            8: "RRF_RT_REG_BINARY",
            16: "RRF_RT_REG_DWORD",
            4: "RRF_RT_REG_EXPAND_SZ",
            32: "RRF_RT_REG_MULTI_SZ",
            1: "RRF_RT_REG_NONE",
            64: "RRF_RT_REG_QWORD",
            2: "RRF_RT_REG_SZ",
            268435456: "RRF_NOEXPAND",
            536870912: "RRF_ZEROONFAILURE",
            65536: "RRF_SUBKEY_WOW6464KEY",
        ...
        if keyValue is not None:
            ...
            try:
                ...
                if type == RegValueTypes.REG_BINARY:
                    uc.mem_write(
                        pVals[5], pack(f"<{len(keyValue.data)}s", keyValue.data)
                    )
                    ...
                    uc.mem_write(
                        pVals[5],
                        pack(
                            f"<{len(keyValue.dataAsStr) + 1}s",
                            keyValue.dataAsStr.encode("ascii"),
                        ),
                    )
                    ...
                    uc.mem_write(
                        pVals[5],
                        pack(
                            f"<{(len(keyValue.dataAsStr) * 2) + 2}s",
                            keyValue.dataAsStr.encode("utf-16"),
                        ),
                    )
                    uc.mem_write(
                        pVals[6], pack("<I", (len(keyValue.dataAsStr) * 2) + 2)
                    )
        ...
        if lpSubKey == "[NULL]":
            ...
            art.registry_edit_keys.add(
                (keyPath, written_values.name, written_values.dataAsStr)
            )
    ...
    def RegQueryValueA(
        ...
        if keyValue is not None:
            ...
            try:
                uc.mem_write(
                    pVals[2],
                    pack(f"<{len(keyValue.data)}s", keyValue.dataAsStr.encode("ascii")),
                )
        ...
        if keyValue is not None:
            ...
            try:
                uc.mem_write(
                    pVals[2],
                    pack(
                        f"<{len(keyValue.data) * 2}s",
                        keyValue.dataAsStr.encode("utf-16")[2:],
                    ),
                )
    ...
    def RegQueryValueExA(
        ...
        if keyValue is not None:
            ...
            try:
                ...
                if type == RegValueTypes.REG_BINARY:
                    uc.mem_write(
                        pVals[4], pack(f"<{len(keyValue.data)}s", keyValue.data)
                    )
                    ...
                    uc.mem_write(
                        pVals[5], pack("<I", (len(keyValue.dataAsStr) * 2) + 2)
                    )
                elif type == RegValueTypes.REG_NONE:
                    uc.mem_write(
                        pVals[4],
        ...
        if pVals[0] in HandlesDict:  # Handle Not Found
            ...
            if hKey.name in RegistryKeys:
                ...
                if valType == RegValueTypes.REG_BINARY:
                    ...
                    valType == RegValueTypes.REG_QWORD
                ):  # Needs to Be Tested with 64bit
                    if em.arch == 32:
                ...
                art.registry_edit_keys.add(
                    (
                        registry_key_address.path,
                        written_values.name,
                        written_values.dataAsStr,
                    )
                )
            else:  # Key Not Found
    ...
    def RegSetKeyValueA(
        ...
        written_values = registry_key_address.getValue()
        art.registry_edit_keys.add(
            (registry_key_address.path, written_values.name, written_values.dataAsStr)
        )
    ...
    def RegCopyTreeA(
        ...
        def updateChildKeys(key: RegKey, oldPath: str, newPath: str):
            ...
            key.handle = Handle(
                HandleType.HKEY, name=key.path, handleValue=RegKey.nextHandleValue
            )
            ...
            for key in keysToCopy:
                if isinstance(key, RegKey):
                    if key.path == keyPath:  # Update Exact Key
                        ...
                        keyCopy.handle = Handle(
                            HandleType.HKEY,
                            name=keyCopy.path,
                            handleValue=RegKey.nextHandleValue,
                        )
                        RegKey.nextHandleValue += 8
                ...
                if pVals[1] < len(rKey.childKeys):
                    ...
                    try:
                        uc.mem_write(
                            pVals[2],
                            pack(
                                f"<{len(childKey.name) + 1}s",
                                childKey.name.encode("ascii"),
                            ),
                        )
                    except:
                        pass
                ...
                if pVals[1] < len(rKey.childKeys):
                    ...
                    try:
                        uc.mem_write(
                            pVals[2],
                            pack(
                                f"<{(len(childKey.name) * 2) + 1}s",
                                childKey.name.encode("utf-16")[2:],
                            ),
                        )
                    except:
                        pass
                ...
                if pVals[1] < len(rKey.values):
                    ...
                    try:
                        uc.mem_write(
                            pVals[2],
                            pack(
                                f"<{len(keyValue.name) + 1}s",
                                keyValue.name.encode("ascii"),
                            ),
                        )
                        ...
                        if type == RegValueTypes.REG_BINARY:
                            uc.mem_write(
                                pVals[6], pack(f"<{len(keyValue.data)}s", keyValue.data)
                            )
                            ...
                            uc.mem_write(
                                pVals[7], pack("<I", len(keyValue.dataAsStr) + 1)
                            )
                        elif type == RegValueTypes.REG_EXPAND_SZ:
                            uc.mem_write(
                                pVals[6],
                            ...
                            uc.mem_write(
                                pVals[7], pack("<I", (len(keyValue.dataAsStr) * 2) + 2)
                            )
                        elif type == RegValueTypes.REG_NONE:
                            uc.mem_write(
                                pVals[6],
                ...
                if pVals[1] < len(rKey.values):
                    ...
                    try:
                        uc.mem_write(
                            pVals[2],
                            pack(
                                f"<{(len(keyValue.name) * 2) + 2}s",
                                keyValue.name.encode("utf-16")[2:],
                            ),
                        )
                        uc.mem_write(pVals[5], pack("<I", keyValue.type.value))
                        type = keyValue.type
    ...
    def RegRestoreKeyA(
        ...
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        dwFlagsReverseLookUp = {
            0x00000008: "REG_FORCE_RESTORE",
            0x00000001: "REG_WHOLE_HIVE_VOLATILE",
        }
        ...
        if pVals[0] in HandlesDict:
            ...
            if hKey.name in RegistryKeys:
                ...
                try:
                    uc.mem_write(
                        pVals[1], pack("<I", 0)
                    )  # Class Name Don't Do Reg Classes Currently
                    uc.mem_write(pVals[2], pack("<I", 0))  # Class Length
                    uc.mem_write(
                        pVals[4], pack("<I", len(rKey.childKeys))
                    )  # SubKey Count
                    maxSubKey = 0
    ...
    def SetWindowsHookExA(
        ...
        idHookReverseLookUp = {
            4: "WH_CALLWNDPROC",
            18: "WH_CALLWNDPROCRET",
            5: "WH_CBT",
            9: "WH_DEBUG",
            17: "WH_FOREGROUNDIDLE",
            3: "WH_GETMESSAGE",
    ...
    def ReplaceFileA(
        ...
        dwReplaceFlagsReverseLookUp = {
            1: "REPLACEFILE_WRITE_THROUGH",
            2: "REPLACEFILE_IGNORE_MERGE_ERRORS",
    ...
    def InternetCombineUrlA(
        ...
        dwFlagsReverseLookUp = {
            536870912: "ICU_NO_ENCODE",
            268435456: "ICU_DECODE",
            134217728: "ICU_NO_META",
            67108864: "ICU_ENCODE_SPACES_ONLY",
            33554432: "ICU_BROWSER_MODE",
        ...
        try:
            uc.mem_write(
                pVals[2], pack(f"<{len(combinedUrl) + 1}s", combinedUrl.encode("ascii"))
            )
        except:
            pass
    ...
    def InternetCombineUrlW(
        ...
        try:
            uc.mem_write(
                pVals[2],
                pack(
                    f"<{(len(combinedUrl) * 2) + 2}s", combinedUrl.encode("utf-16")[2:]
                ),
            )
        except:
            pass
    ...
    def ExitWindowsEx(
        ...
        dwReasonReverseLookUp = {
            262144: "SHTDN_REASON_MAJOR_APPLICATION",
            65536: "SHTDN_REASON_MAJOR_HARDWARE",
            458752: "SHTDN_REASON_MAJOR_LEGACY_API",
            131072: "SHTDN_REASON_MAJOR_OPERATINGSYSTEM",
            0: "SHTDN_REASON_MINOR_OTHER",
            393216: "SHTDN_REASON_MAJOR_POWER",
            196608: "SHTDN_REASON_MAJOR_SOFTWARE",
            327680: "SHTDN_REASON_MAJOR_SYSTEM",
            15: "SHTDN_REASON_MINOR_BLUESCREEN",
            11: "SHTDN_REASON_MINOR_CORDUNPLUGGED",
            7: "SHTDN_REASON_MINOR_DISK",
            12: "SHTDN_REASON_MINOR_ENVIRONMENT",
            13: "SHTDN_REASON_MINOR_HARDWARE_DRIVER",
            17: "SHTDN_REASON_MINOR_HOTFIX",
            23: "SHTDN_REASON_MINOR_HOTFIX_UNINSTALL",
            5: "SHTDN_REASON_MINOR_HUNG",
            2: "SHTDN_REASON_MINOR_INSTALLATION",
            1: "SHTDN_REASON_MINOR_MAINTENANCE",
            25: "SHTDN_REASON_MINOR_MMC",
            20: "SHTDN_REASON_MINOR_NETWORK_CONNECTIVITY",
            9: "SHTDN_REASON_MINOR_NETWORKCARD",
            14: "SHTDN_REASON_MINOR_OTHERDRIVER",
            10: "SHTDN_REASON_MINOR_POWER_SUPPLY",
            8: "SHTDN_REASON_MINOR_PROCESSOR",
            4: "SHTDN_REASON_MINOR_RECONFIG",
            19: "SHTDN_REASON_MINOR_SECURITY",
            18: "SHTDN_REASON_MINOR_SECURITYFIX",
            24: "SHTDN_REASON_MINOR_SECURITYFIX_UNINSTALL",
            16: "SHTDN_REASON_MINOR_SERVICEPACK",
            22: "SHTDN_REASON_MINOR_SERVICEPACK_UNINSTALL",
            32: "SHTDN_REASON_MINOR_TERMSRV",
            6: "SHTDN_REASON_MINOR_UNSTABLE",
            3: "SHTDN_REASON_MINOR_UPGRADE",
            21: "SHTDN_REASON_MINOR_WMI",
            1073741824: "SHTDN_REASON_FLAG_USER_DEFINED",
            2147483648: "SHTDN_REASON_FLAG_PLANNED",
        }
    ...
    def CreateFileMappingA(
        ...
        flProtectReverseLookUp = {
            ...
            4: "PAGE_READWRITE",
            8: "PAGE_WRITECOPY",
            134217728: "SEC_COMMIT",
            16777216: "SEC_IMAGE",
            285212672: "SEC_IMAGE_NO_EXECUTE",
            2147483648: "SEC_LARGE_PAGES",
            268435456: "SEC_NOCACHE",
            67108864: "SEC_RESERVE",
    ...
    def CryptAcquireContextA(
        ...
        dwProvTypeReverseLookUp = {
            ...
            5: "PROV_MS_EXCHANGE",
            6: "PROV_SSL",
            18: "PROV_RSA_SCHANNEL",
            19: "PROV_DSS_DH",
            24: "PROV_DH_SCHANNEL",
            36: "PROV_RSA_AES",
        }
        dwFlagsReverseLookUp = {
            4026531840: "CRYPT_VERIFYCONTEXT",
            8: "CRYPT_NEWKEYSET",
            16: "CRYPT_DELETEKEYSET",
            32: "CRYPT_MACHINE_KEYSET",
            64: "CRYPT_SILENT",
            128: "CRYPT_DEFAULT_CONTAINER_OPTIONAL",
        }
    ...
    def OpenSCManagerA(
        ...
        dwDesiredAccessReverseLookUp = {
            ...
            8: "SC_MANAGER_LOCK",
            32: "SC_MANAGER_MODIFY_BOOT_CONFIG",
    ...
    def InternetQueryOptionA(
        ...
        dwOptionReverseLookUp = {
            ...
            105: "INTERNET_OPTION_CACHE_TIMESTAMPS",
            ...
            51: "INTERNET_OPTION_DATAFILE_NAME",
            103: "INTERNET_OPTION_DIAGNOSTIC_SOCKET_INFO",
            ...
            36: "INTERNET_OPTION_EXTENDED_ERROR",
            ...
            33: "INTERNET_OPTION_PARENT_HANDLE",
    ...
    def HttpOpenRequestA(
        ...
        dwFlagsReverseLookUp = {
            65536: "INTERNET_FLAG_CACHE_IF_NET_FAIL",
            1024: "INTERNET_FLAG_HYPERLINK",
            ...
            2097152: "INTERNET_FLAG_NO_AUTO_REDIRECT",
            67108864: "INTERNET_FLAG_NO_CACHE_WRITE",
            524288: "INTERNET_FLAG_NO_COOKIES",
            512: "INTERNET_FLAG_NO_UI",
            256: "INTERNET_FLAG_PRAGMA_NOCACHE",
            2147483648: "INTERNET_FLAG_RELOAD",
            2048: "INTERNET_FLAG_RESYNCHRONIZE",
            8388608: "INTERNET_FLAG_SECURE",
        }
    ...
    def HttpAddRequestHeadersA(
        ...
        dwModifiersReverseLookUp = {
            536870912: "HTTP_ADDREQ_FLAG_ADD",
            268435456: "HTTP_ADDREQ_FLAG_ADD_IF_NEW",
    ...
    def HttpQueryInfoA(
        ...
        dwInfoLevelReverseLookUp = {
            36: "HTTP_QUERY_ACCEPT",
            37: "HTTP_QUERY_ACCEPT_CHARSET",
            38: "HTTP_QUERY_ACCEPT_ENCODING",
            39: "HTTP_QUERY_ACCEPT_LANGUAGE",
            66: "HTTP_QUERY_ACCEPT_RANGES",
            72: "HTTP_QUERY_AGE",
            7: "HTTP_QUERY_ALLOW",
            40: "HTTP_QUERY_AUTHORIZATION",
            73: "HTTP_QUERY_CACHE_CONTROL",
            35: "HTTP_QUERY_CONNECTION",
            80: "HTTP_QUERY_CONTENT_BASE",
            4: "HTTP_QUERY_CONTENT_DESCRIPTION",
            71: "HTTP_QUERY_CONTENT_DISPOSITION",
            41: "HTTP_QUERY_CONTENT_ENCODING",
            3: "HTTP_QUERY_CONTENT_ID",
            6: "HTTP_QUERY_CONTENT_LANGUAGE",
            5: "HTTP_QUERY_CONTENT_LENGTH",
            81: "HTTP_QUERY_CONTENT_LOCATION",
            82: "HTTP_QUERY_CONTENT_MD5",
            83: "HTTP_QUERY_CONTENT_RANGE",
            2: "HTTP_QUERY_CONTENT_TRANSFER_ENCODING",
            1: "HTTP_QUERY_CONTENT_TYPE",
            68: "HTTP_QUERY_COOKIE",
            21: "HTTP_QUERY_COST",
            415029: "HTTP_QUERY_CUSTOM",
            9: "HTTP_QUERY_DATE",
            20: "HTTP_QUERY_DERIVED_FROM",
            115: "HTTP_QUERY_ECHO_HEADERS",
            116: "HTTP_QUERY_ECHO_HEADERS_CRLF",
            114: "HTTP_QUERY_ECHO_REPLY",
            113: "HTTP_QUERY_ECHO_REQUEST",
            84: "HTTP_QUERY_ETAG",
            104: "HTTP_QUERY_EXPECT",
            16: "HTTP_QUERY_EXPIRES",
            48: "HTTP_QUERY_FORWARDED",
            49: "HTTP_QUERY_FROM",
            85: "HTTP_QUERY_HOST",
            86: "HTTP_QUERY_IF_MATCH",
            50: "HTTP_QUERY_IF_MODIFIED_SINCE",
            87: "HTTP_QUERY_IF_NONE_MATCH",
            88: "HTTP_QUERY_IF_RANGE",
            89: "HTTP_QUERY_IF_UNMODIFIED_SINCE",
            17: "HTTP_QUERY_LAST_MODIFIED",
            22: "HTTP_QUERY_LINK",
            51: "HTTP_QUERY_LOCATION",
            120: "HTTP_QUERY_MAX",
            96: "HTTP_QUERY_MAX_FORWARDS",
            18: "HTTP_QUERY_MESSAGE_ID",
            0: "HTTP_QUERY_MIME_VERSION",
            52: "HTTP_QUERY_ORIG_URI",
            23: "HTTP_QUERY_PRAGMA",
            65: "HTTP_QUERY_PROXY_AUTHENTICATE",
            97: "HTTP_QUERY_PROXY_AUTHORIZATION",
            105: "HTTP_QUERY_PROXY_CONNECTION",
            8: "HTTP_QUERY_PUBLIC",
            98: "HTTP_QUERY_RANGE",
            33: "HTTP_QUERY_RAW_HEADERS",
            34: "HTTP_QUERY_RAW_HEADERS_CRLF",
            53: "HTTP_QUERY_REFERER",
            70: "HTTP_QUERY_REFRESH",
            69: "HTTP_QUERY_REQUEST_METHOD",
            54: "HTTP_QUERY_RETRY_AFTER",
            55: "HTTP_QUERY_SERVER",
            67: "HTTP_QUERY_SET_COOKIE",
            25: "HTTP_QUERY_STATUS_CODE",
            32: "HTTP_QUERY_STATUS_TEXT",
            56: "HTTP_QUERY_TITLE",
            99: "HTTP_QUERY_TRANSFER_ENCODING",
            112: "HTTP_QUERY_UNLESS_MODIFIED_SINCE",
            100: "HTTP_QUERY_UPGRADE",
            19: "HTTP_QUERY_URI",
            57: "HTTP_QUERY_USER_AGENT",
            101: "HTTP_QUERY_VARY",
            24: "HTTP_QUERY_VERSION",
            102: "HTTP_QUERY_VIA",
            103: "HTTP_QUERY_WARNING",
            64: "HTTP_QUERY_WWW_AUTHENTICATE",
            121: "HTTP_QUERY_X_CONTENT_TYPE_OPTIONS",
            128: "HTTP_QUERY_P3P",
            129: "HTTP_QUERY_X_P2P_PEERDIST",
            130: "HTTP_QUERY_TRANSLATE",
            131: "HTTP_QUERY_X_UA_COMPATIBLE",
            132: "HTTP_QUERY_DEFAULT_STYLE",
            133: "HTTP_QUERY_X_FRAME_OPTIONS",
            134: "HTTP_QUERY_X_XSS_PROTECTION",
            268435456: "HTTP_QUERY_FLAG_COALESCE",
            536870912: "HTTP_QUERY_FLAG_NUMBER",
    ...
    def InternetOpenUrlA(
        ...
        dwFlagsReverseLookUp = {
            ...
            134217728: "INTERNET_FLAG_PASSIVE",
            256: "INTERNET_FLAG_PRAGMA_NOCACHE",
            1073741824: "INTERNET_FLAG_RAW_DATA",
            2147483648: "INTERNET_FLAG_RELOAD",
            2048: "INTERNET_FLAG_RESYNCHRONIZE",
            8388608: "INTERNET_FLAG_SECURE",
        }
    ...
    def MoveFileExA(
        ...
        if folder:
            originPath, destinationPath, originFileName, destinationFileName = (
                SimFileSystem.moveFile(origin, destination, replace)
            )
            ...
            originPath, destinationPath = SimFileSystem.moveFolder(
                origin, destination, replace
            )
            art.path_artifacts.append(originPath)
            art.path_artifacts.append(destinationPath)
    ...
    def CopyFileExA(
        ...
        mdwCopyFlagsReverseLookUp = {
            8: "COPY_FILE_ALLOW_DECRYPTED_DESTINATION",
            2048: "COPY_FILE_COPY_SYMLINK",
            1: "COPY_FILE_FAIL_IF_EXISTS",
            4096: "COPY_FILE_NO_BUFFERING",
            4: "COPY_FILE_OPEN_SOURCE_FOR_WRITE",
            2: "COPY_FILE_RESTARTABLE",
        ...
        origin = pVals[0]
        destination = pVals[1]
        originFileName, destinationFileName, originPath, destinationPath = (
            SimFileSystem.copyFile(origin, destination)
        )
        art.path_artifacts.append(originPath)
        art.path_artifacts.append(destinationPath)
    ...
    def WideCharToMultiByte(
        ...
        try:
            ...
            ascii = stringToConvert.encode(
                "ascii", errors="replace"
            )  # Attempt to encode replace unknown with ?
            if pVals[5] == 0:
                retVal = len(ascii) + 1
            else:
    ...
    def GetSystemWindowsDirectoryA(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
    ...
    def GetSystemWindowsDirectoryW(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
    ...
    def SetErrorMode(
        ...
        pNames = ["uMode"]
        pVals = makeArgVals(uc, em, esp, len(pTypes))
        SetErrorModeFormatReverseLookup = {
            0: "",
            1: "SEM_FAILCRITICALERRORS",
            4: "SEM_NOALIGNMENTFAULTEXCEPT",
            2: "SEM_NOGPFAULTERRORBOX",
            32768: "SEM_NOOPENFILEERRORBOX",
        }
    ...
    def TlsFree(
        ...
        retValStr = "TRUE"
        uc.reg_write(
            UC_X86_REG_EAX, retVal
        )  ## The return value can be of 4 differnt things, what do i do in this situation?
    ...
    def CreateFile2(
        ...
        pTypes = [
            ...
            "DWORD",
            "DWORD",
            "LPSCREATEFILE2_EXTENDED_PARAMETERS",
        ]
        pNames = [
            "lpFileName",
            "dwDesiredAccess",
    ...
    def lstrcatA(
        ...
        try:
            finalStr = str1 + str2
            uc.mem_write(
                pVals[0], pack(f"<{len(finalStr) + 1}s", finalStr.encode("ascii"))
            )
        except:
            pass
    ...
    def lstrcatW(
        ...
        try:
            ...
            uc.mem_write(
                pVals[0],
                pack(f"<{(len(finalStr) * 2) + 2}s", finalStr.encode("utf-16")),
            )
        except:
            pass
    ...
    def lstrcpynW(
        ...
        try:
            uc.mem_write(
                pVals[0], pack(f"<{pVals[2] * 2}s", string2.encode("utf-16")[2:])
            )
        except:
            pass
    ...
    def lstrcpyA(
        ...
        try:
            uc.mem_write(
                pVals[0], pack(f"<{len(string2) + 1}s", string2.encode("ascii"))
            )
        except:
            pass
    ...
    def lstrcpyW(
        ...
        try:
            uc.mem_write(
                pVals[0],
                pack(f"<{len(string2) * 2 + 2}s", string2.encode("utf-16")[2:]),
            )
        except:
            pass
    ...
    def GetTimeZoneInformation(
        ...
        if pVals[0] != 0x0:
            ...
            timeZone.DaylightDate.setTime(
                False, conr.simulatedValues_system_time_since_epoch
            )
            timeZone.StandardDate.setTime(
                False, conr.simulatedValues_system_time_since_epoch
            )
    ...
    def GetStartupInfoA(
        ...
        if pVals[0] != 0x0:
            ...
            uc.mem_write(
                ...
                pack(
                    f"<{len(conr.simulatedValues_computer_name) + 1}s",
                    conr.simulatedValues_computer_name.encode("utf-16")[2:],
                ),
            )
    ...
    def GetStartupInfoW(
        ...
        if pVals[0] != 0x0:
            ...
            uc.mem_write(
                startupinfo.lpDesktop,
                pack(
                    f"<{len(conr.simulatedValues_computer_name) * 2 + 2}s",
                    conr.simulatedValues_computer_name.encode("utf-16")[2:],
                ),
            )
            startupinfo.writeToMemory(uc, pVals[0])
    ...
    def EnumProcessModulesEx(
        ...
        dwFilterFlag_ReverseLookUp = {
            1: "LIST_MODULES_32BIT",
            2: "LIST_MODULES_64BIT",
            3: "LIST_MODULES_ALL",
            0: "LIST_MODULES_DEFAULT",
        }
    ...
    def IsProcessorFeaturePresent(
        ...
        ProcessorFeature_ReverseLookUp = {
    ...
    def xxCryptStringToBinaryA(
        ...
        dwFlagsReverseLookUp = {
            ...
            2: "CRYPT_STRING_BINARY",
            3: "CRYPT_STRING_BASE64REQUESTHEADER",
            4: "CRYPT_STRING_HEX",
            5: "CRYPT_STRING_HEXASCII",
            6: "CRYPT_STRING_BASE64_ANY",
            7: "CRYPT_STRING_ANY",
            8: "CRYPT_STRING_HEX_ANY",
            9: "CRYPT_STRING_BASE64X509CRLHEADER",
            10: "CRYPT_STRING_HEXADDR",
            11: "CRYPT_STRING_HEXASCIIADDR",
            12: "CRYPT_STRING_HEXRAW",
            536870912: "CRYPT_STRING_STRICT",
        }
    ...
    def xxCryptBinaryToStringA(
        ...
        dwFlagsReverseLookUp = {
            ...
            12: "CRYPT_STRING_HEXRAW",
            536870912: "CRYPT_STRING_STRICT",
            1073741824: "CRYPT_STRING_NOCRLF",
            2147483648: "CRYPT_STRING_NOCR",
        }
    ...
    def xxNetShareGetInfo(
        ...
        dwFlagsReverseLookUp1 = {
            0: "SHARE_NAME",
            1: "SHARED_RESOURCE_INFO",
            2: "SHARED_RESOURCE_INFO",
            501: "NAME_TYPE_RESOURCE",
            502: "PERMS_RESOURCE_CONNECTIONS",
            503: "PERMS_RESOURCE_CONNECTIONS",
            1005: "SHARE_ROOT_VOLUME_INDICATOR",
        }
    ...
    def xxVirtualAllocExNuma(
        ...
        dwFlagsReverseLookUp = {
            ...
            8192: "MEM_RESERVE",
            524288: "MEM_RESET",
            16777216: "MEM_RESET_UNDO",
            536870912: "MEM_LARGE_PAGES",
            4194304: "MEM_PHYSICAL",
            1048576: "MEM_TOP_DOWN",
        }
    ...
    def xxWNetAddConnection2A(
        ...
        dwFlagsReverseLookUp = {
            ...
            2: "CONNECT_UPDATE_RECENT",
            4: "CONNECT_TEMPORARY",
            8: "CONNECT_INTERACTIVE",
            16: "CONNECT_PROMPT",
            128: "CONNECT_REDIRECT",
            512: "CONNECT_CURRENT_MEDIA",
            2048: "CONNECT_COMMANDLINE",
            4096: "CONNECT_CMD_SAVECRED",
            8192: "CONNECT_CRED_RESET",
        }
    ...
    def GetThreadInformation(
        ...
        pVals[1] = getLookUpVal(
            pVals[1], ReverseLookUps.Thread.THREAD_INFORMATION_CLASS
        )
        ...
        if pVals[0] in HandlesDict:
            ...
            if handle.type == HandleType.HMODULE:
                if handle.name != "":
                    uc.mem_write(
                        pVals[0],
                        pack(f"<{len(handle.name) + 1}s", handle.name.encode("ascii")),
                    )
    ...
    def MoveFileA(
        ...
        destination = pVals[1]
        originPath, destinationPath, originFileName, destinationFileName = (
            SimFileSystem.moveFile(origin, destination, 0x1)
        )
        art.path_artifacts.append(originPath)
        art.path_artifacts.append(destinationPath)
        # art.file_artifacts.append(originFileName)
    ...
    def GetFileType(
        ...
        if pVals[0] in HandlesDict:
            ...
            if (
                fileTypeHandle.type == HandleType.CreateFileW
                or fileTypeHandle.type == HandleType.CreateFileA
                or fileTypeHandle.type == HandleType.CreateFile2
            ):
                retVal = 0x0001
                retValStr = "FILE_TYPE_DISK"
            elif (
                fileTypeHandle.type == HandleType.FtpOpenFileA
                or fileTypeHandle.type == HandleType.FtpOpenFileW
            ):
                retVal = 0x8000
    ...
    def GlobalLock(
        ...
        if pVals[0] in HandlesDict:
            ...
            uc.mem_write(
                allocMemoryVal,
                pack(f"<{len(handle.data)}s", handle.data.encode("ascii")),
            )
    ...
    def SetWindowLongPtrA(
        ...
        setWindowLongPtrA_ReverseLookUp = {
            -32: "GWL_EXSTYLE",
            -6: "GWLP_HINSTANCE",
            -18: "GWLP_ID",
            -22: "GWL_STYLE",
    ...
    def MapVirtualKeyA(
        ...
        # pTypes,pVals= findStringsParms(uc, pTypes,pVals, skip=[1])
        pVals[1] = getLookUpVal(
            pVals[1], ReverseLookUps.KeyBoard.MapKey_MapType_ReverseLookUp
        )
        uc.reg_write(UC_X86_REG_EAX, retVal)
    ...
    def LoadKeyboardLayoutA(
        ...
        keyboardIdentifierStr = getLookUpVal(
            keyboardIdentifier, ReverseLookUps.KeyBoard.keyboardLanguages
        )
        pVals[1] = getLookUpVal(
            pVals[1], ReverseLookUps.KeyBoard.loadKeyboard_uFlags_ReverseLookUp
        )
    ...
    def LoadKeyboardLayoutW(
        ...
        keyboardIdentifier = int(pVals[0], 16)
        keyboardIdentifierStr = getLookUpVal(
            keyboardIdentifier, ReverseLookUps.keyboardLanguages
        )
        pVals[1] = getLookUpVal(
            pVals[1], ReverseLookUps.KeyBoard.loadKeyboard_uFlags_ReverseLookUp
        )
    ...
    def CreateWaitableTimerExW(
        ...
        dwFlags_ReverseLookUp = {
            1: "CREATE_WAITABLE_TIMER_MANUAL_RESET",
            2: "CREATE_WAITABLE_TIMER_HIGH_RESOLUTION",
        }
        timerAccessRights_ReverseLookUp = {
            2031619: "TIMER_ALL_ACCESS",
            2: "TIMER_MODIFY_STATE",
            1: "TIMER_QUERY_STATE",
        }
    ...
    def CreateTimerQueueTimer(
        ...
        Flags_ReverseLookUp = {
            0: "WT_EXECUTEDEFAULT",
            32: "WT_EXECUTEINTIMERTHREAD",
            1: "WT_EXECUTEINIOTHREAD",
            128: "WT_EXECUTEINPERSISTENTTHREAD",
            16: "WT_EXECUTELONGFUNCTION",
            8: "WT_EXECUTEONLYONCE",
            256: "WT_TRANSFER_IMPERSONATION",
        }
    ...
    def NtCreateProcess(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCreateProcess
        )
    ...
    def NtTerminateProcess(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtTerminateProcess
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtCreateThread(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCreateThread
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtCreateThreadEx(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCreateThreadEx
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtTerminateThread(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtTerminateThread
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtAllocateVirtualMemory(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtAllocateVirtualMemory
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtWriteVirtualMemory(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtWriteVirtualMemory
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtReadVirtualMemory(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtReadVirtualMemory
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtShutdownSystem(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtShutdownSystem
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtCreateNamedPipeFile(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCreateNamedPipeFile
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtCreateKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCreateKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtOpenKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtOpenKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtOpenKeyEx(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtOpenKeyEx
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtSetValueKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtSetValueKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtDeleteValueKey(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtDeleteValueKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtDeleteKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtDeleteKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtQueryKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtQueryKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtEnumerateKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtEnumerateKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtQueryValueKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtQueryValueKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtEnumerateValueKey(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtEnumerateValueKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtFlushKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtFlushKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtCompressKey(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtCompressKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtFreezeRegistry(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtFreezeRegistry
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtThawRegistry(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtThawRegistry
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtLockRegistryKey(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtLockRegistryKey
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtClose(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtClose
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtQuerySystemTime(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtQuerySystemTime
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))

    def NtQueryPerformanceCounter(
        self, uc: Uc, eip: int, esp: int, export_dict: dict, callAddr: int, em: EMU
    ):
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtQueryPerformanceCounter
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtDisplayString(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtDisplayString
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtDrawText(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtDrawText
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtAddAtom(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtAddAtom
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtDeleteAtom(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtDeleteAtom
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtFindAtom(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtFindAtom
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
    ...
    def NtLockFile(
        ...
        logged_calls = CustomWinSysCalls().winApiToSyscall(
            uc, eip, esp, callAddr, em, CustomWinSysCalls().NtLockFile
        )
        return logged_calls, stackCleanup(uc, em, esp, len(logged_calls[5]))
...
class CustomWinSysCalls:
    def winApiToSyscall(
        ...
        callAddr: int,
        em: EMU,
        syscall: Callable[[Uc, int, int, int, EMU], list],
    ):
        if em.arch == 32:
            if em.winVersion == "Windows 7":
                esp = uc.reg_write(
                    UC_X86_REG_EDX, (esp + 4)
                )  # Params start at value in edx
    ...
    def NtAllocateVirtualMemory(
        self, uc: Uc, eip: int, esp: int, callAddr: int, em: EMU
    ):
        ...
        try:
            ...
            try:
                ...
                uc.reg_write(UC_X86_REG_EAX, retVal)
                uc.mem_write(
                    pVals[1], pack("<Q", allocLoc)
                )  # Write Allocation Location
    ...
    def NtCreateNamedPipeFile(self, uc: Uc, eip: int, esp: int, callAddr: int, em: EMU):
        ...
        desiredAccessReverseLookUp = {
            ...
            0x00200000: "READ_CONTROL",
            0x00080000: "WRITE_OWNER",
            0x00040000: "WRITE_DAC",
            0x01000000: "ACCESS_SYSTEM_SECURITY",
        }
        shareAccessReverseLookUp = {
            0x1: "FILE_SHARE_READ",
            0x2: "FILE_SHARE_WRITE",
            0x4: "FILE_SHARE_DELETE",
        }
        createDispositionReverseLookUp = {
        ...
        createOptionsReverseLookup = {
            1: "FILE_DIRECTORY_FILE",
            2: "FILE_WRITE_THROUGH",
            4: "FILE_SEQUENTIAL_ONLY",
            8: "FILE_NO_INTERMEDIATE_BUFFERING",
            16: "FILE_SYNCHRONOUS_IO_ALERT",
            32: "FILE_SYNCHRONOUS_IO_NONALERT",
            64: "FILE_NON_DIRECTORY_FILE",
            128: "FILE_CREATE_TREE_CONNECTION",
            256: "FILE_COMPLETE_IF_OPLOCKED",
            512: "FILE_NO_EA_KNOWLEDGE",
            1024: "FILE_OPEN_REMOTE_INSTANCE",
            2048: "FILE_RANDOM_ACCESS",
            4096: "FILE_DELETE_ON_CLOSE",
            8192: "FILE_OPEN_BY_FILE_ID",
            16384: "FILE_OPEN_FOR_BACKUP_INTENT",
            32768: "FILE_NO_COMPRESSION",
            65536: "FILE_OPEN_REQUIRING_OPLOCK",
            1048576: "FILE_RESERVE_OPFILTER",
            2097152: "FILE_OPEN_REPARSE_POINT",
            4194304: "FILE_OPEN_NO_RECALL",
            8388608: "FILE_OPEN_FOR_FREE_SPACE_QUERY",
            16777215: "FILE_VALID_OPTION_FLAGS",
            50: "FILE_VALID_MAILSLOT_OPTION_FLAGS",
            54: "FILE_VALID_SET_FLAGS",
        }
    ...
    def NtOpenKeyEx(self, uc: Uc, eip: int, esp: int, callAddr: int, em: EMU):
        ...
        openOptions = {
            4: "REG_OPTION_BACKUP_RESTORE",
            8: "REG_OPTION_OPEN_LINK",
            12: "REG_OPTION_OPEN_LINK | REG_OPTION_BACKUP_RESTORE",
        }
        ...
        if pVals[0] in HandlesDict:
            ...
            if hKey.name in RegistryKeys:
                ...
                if pVals[1] == "KeyBasicInformation":
                    requiredSize = sizeof(KEY_BASIC_INFORMATION) + len(
                        rKey.name.encode("utf-16")[2:]
                    )
                    ...
                    requiredSize = sizeof(KEY_NODE_INFORMATION) + len(
                        rKey.name.encode("utf-16")[2:]
                    )
                    if pVals[3] >= requiredSize and pVals[2] != 0x0:
                ...
                if pVals[1] < len(rKey.childKeys):
                    ...
                    if pVals[2] == "KeyBasicInformation":
                        ...
                        if pVals[4] >= requiredSize and pVals[3] != 0x0:
                            ...
                            uc.mem_write(
                                nameAddress, childKey.name.encode("utf-16")[2:]
                            )
                            pVals[3] = makeStructVals(uc, info, pVals[3])
            ...
            if hKey.name in RegistryKeys:
                ...
                if pVals[1] != 0x0:
                    ...
                    if pVals[2] == "KeyValueBasicInformation":
                        requiredSize = sizeof(KEY_VALUE_BASIC_INFORMATION) + len(
                            keyValue.name.encode("utf-16")[2:]
                        )
                        if pVals[4] >= requiredSize and pVals[3] != 0x0:
                            ...
                            info.writeToMemory(uc, pVals[3])
                            nameAddress = (
                                pVals[3] + KEY_VALUE_BASIC_INFORMATION.Name.offset
                            )
                            uc.mem_write(
                                nameAddress, keyValue.name.encode("utf-16")[2:]
                            )
                            pVals[3] = makeStructVals(uc, info, pVals[3])
                        ...
                        if pVals[4] >= requiredSize and pVals[3] != 0x0:
                            ...
                            info.writeToMemory(uc, pVals[3])
                            dataAddress = (
                                pVals[3] + KEY_VALUE_PARTIAL_INFORMATION.Data.offset
                            )
                        ...
                        requiredSize = (
                            ...
                            + len(keyValue.name.encode("utf-16")[2:])
                            + keyValue.dataLength(True)
                        )
                        if pVals[4] >= requiredSize and pVals[3] != 0x0:
                            ...
                            info.Type = keyValue.type.value
                            info.DataOffset = (
                                KEY_VALUE_FULL_INFORMATION.Name.offset
                                + len(keyValue.name.encode("utf-16")[2:])
                            )
                            info.DataLength = keyValue.dataLength(True)
                            info.NameLength = len(keyValue.name.encode("utf-16")[2:])
                            ...
                            info.writeToMemory(uc, pVals[3])
                            nameAddress = (
                                pVals[3] + KEY_VALUE_FULL_INFORMATION.Name.offset
                            )
...
def read_string(uc: Uc, address: int) -> str:
    ...
    if c == 0x0:
        ret = "[NULL]"  # Option for NULL String
...
def writeAsciiStrToMemory(uc: Uc, address: int, string: str):
    uc.mem_write(
        address, pack(f"<{len(string.encode('ascii')) + 2}s", string.encode("ascii"))
    )
...
def writeUnicodeStrToMemory(uc: Uc, address: int, string: str):
    uc.mem_write(
        address,
        pack(f"<{len(string.encode('utf-16')[2:]) + 2}s", string.encode("utf-16")[2:]),
    )
