...
class Artifacts_regex:
    ...
    def Paths(self):
        ...
        self.total_findPaths = (
            find_letterDrives2
            + "|"
            + find_letterDrives2_2
            + "|"
            + find_letterDrives
            + "|"
            + find_letterDrives_2
            ...
            + find_relativePaths
            + "|"
            + find_relativePaths_2
            + "|"
            + find_networkShares
            + "|"
            + find_networkShares_2
            + "|"
            + find_environment
            + "|"
            + find_environment_2
    ...
    def CommandLineArgs(self):
        ...
        self.total_commandLineArguments = (
            cmdline_args
            + "|"
            + powershell_args
            + "|"
    ...
    def Web(self):
        ...
        find_ftp = r"(?:(?:ftp):\/\/(?:[\S]+))"
        find_ipAddress = (
            r"(?:(?:[0-9]{,3}\.[0-9]{,3}\.[0-9]{,3}\.[0-9]{,3})(?:[^\s\'\",]+))"
        )
        self.total_webTraffic = (
            find_website
            + "|"
            + find_ftp
            + "|"
    ...
    def Registry(self):
        find_HKEY = (
            r"(?:(?:HKEY|HKLM|HKCU|HKCC|HKCR|HKU)(?:\:)?(?:[_A-z0-9])+(?:\\[^\\\n]+)+)"
        )
        ...
        find_LocalMachine = (
            r"(?:(?:SOFTWARE|SYSTEM|HARDWARE|SAM|BCD00000000)(?:\\[^\n]+){+)"
        )
...
class Artifacts_emulation:
    ...
    def combineRegexEmuCMDline(self):
        self.commandLine_artifacts = (
            set(self.commandLine_artifacts) | self.commandLine_HookApis
        )
        self.commandLine_artifacts = list(self.commandLine_artifacts)
    ...
    def removeStructures(self, Regex):
        ...
        for each in self.path_artifacts:
            if "', " in each:
                ...
                for next in temp:
                    next = next.strip("'")
                    self.path_artifacts += re.findall(
                        Regex.total_findPaths, next, re.IGNORECASE
                    )
        ...
        for each in self.file_artifacts:
            if "', " in each:
                ...
                for next in temp:
                    next = next.strip("'")
                    self.file_artifacts += re.findall(
                        Regex.find_totalFiles, next, re.IGNORECASE
                    )
        ...
        for each in self.registry_misc:
            if "', " in each:
                ...
                for next in temp:
                    next = next.strip("'")
                    self.registry_artifacts += re.findall(
                        Regex.total_Registry, next, re.IGNORECASE
                    )
        ...
        for each in self.exe_dll_artifacts:
            if "', " in each:
                ...
                for next in temp:
                    next = next.strip("'")
                    self.exe_dll_artifacts += re.findall(
                        Regex.find_exe_dll, next, re.IGNORECASE
                    )
