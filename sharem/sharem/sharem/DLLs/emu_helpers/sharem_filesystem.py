...
class Directory_system:
    ...
    def InitializeFileSystem(self):
        ...
        self.rootDir.childrenDir.update(
            self.CreateNewFolder("Program Files", self.rootDir)
        )
        self.rootDir.childrenDir.update(
            self.CreateNewFolder("Program Files(x86)", self.rootDir)
        )
        self.rootDir.childrenDir.update(
            self.CreateNewFolder("ProgramData", self.rootDir)
        )
        ...
        self.users = (
            Configuration().simulatedValues_users
    ...
    def recurseCreateFolder(self, dirNode, path, i=1):
        ...
        if i >= len(path):
            ...
            if path[i] not in dirNode.childrenDir:
                ...
                return self.recurseCreateFolder(
                    dirNode.childrenDir.get(path[i]), path, i + 1
                )
            else:
                return self.recurseCreateFolder(
                    dirNode.childrenDir.get(path[i]), path, i + 1
