...
class RegKey:
    ...
    nextRemoteHandleValues = (
        0x90000010  # Registry Start value for Remote Computer Handles
    )
    ...
    def setValue(
        self, valueType: RegValueTypes, data: Any, valueName: str = "(Default)"
    ):
    ...
    def printInfo(self):
        ...
        if len(self.values) > 0:
            ...
            for key, val in self.values.items():
                print(
                    "{:<20} {:<20} {:<20}".format(
                        val.name, val.type.name, val.dataAsStr
                    )
                )
            ...
            try:
                if self.type == RegValueTypes.REG_BINARY:
                    ...
                    uc.mem_write(
                        address,
                        pack(
                            f"<{(len(self.dataAsStr) * 2) + 2}s",
                            self.dataAsStr.encode("utf-16")[2:],
                        ),
                    )
            ...
            try:
                if self.type == RegValueTypes.REG_BINARY:
                    ...
                    uc.mem_write(
                        address,
                        pack(
                            f"<{len(self.dataAsStr) + 2}s",
                            self.dataAsStr.encode("ascii"),
                        ),
                    )
                elif self.type == RegValueTypes.REG_EXPAND_SZ:
                    uc.mem_write(
