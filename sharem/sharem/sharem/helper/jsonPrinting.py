...
class jsonPrint:
    ...
    def generateJson(self, filename, peName, sharem_out_dir, rawHex):
        ...
        (
            ...
            defaultMain,
            defaultDiss,
        ) = self.createFileNames(fileNameTime, filename, peName, sharem_out_dir)
    ...
    def apis(self, apiList):
        ...
        for i in apiList:
            ...
            if tuple_flag == 1:
                api_dict.update(
                    self.jsonTuples(
                        api_params_values, api_params_types, api_params_names, api_dict
                    )
                )
            else:
                ...
                for pName in api_params_names:
                    api_type_value = []
                    api_type_value.append(
                        {
                            "type": api_params_types[p],
                            "value": str(api_params_values[p]),
                        }
                    )
                    api_dict["parameters"].append(
                        {"type": api_params_names[p], "value": api_type_value}
                    )
                    p += 1
        ...
        for i in var.logged_syscalls:
            ...
            if tuple_flag == 1:
                ...
                for pName in syscall_params_names:
                    syscall_type_value = []
                    syscall_type_value.append(
                        {
                            "type": syscall_params_types[p],
                            "value": str(syscall_params_values[p]),
                        }
                    )
                    syscalls_dict["parameters"].append(
                        {"type": syscall_params_names[p], "value": syscall_type_value}
                    )
                    p += 1
            ...
            syscalls_dict["OS_Release_SP"] = (
                var.emu.winVersion + ", SP " + var.emu.winSP
            )
    ...
    def registryActions(self, art):
        ...
        for i in art.registry_add_keys:
            addedKeysList.append({"key_path": i, "value": ""})
        for i in art.registry_edit_keys:
            editedKeysList.append({"key_path": i[0], "value": str([i[1], i[2]])})
        for i in art.registry_delete_keys:
            if type(i) == tuple:
                deletedKeysList.append({"key_path": i[0], "value": str(i[1])})
            else:
                deletedKeysList.append({"key_path": i, "value": ""})
        ...
        for pv in paramValues:
            if isinstance(paramValues[t], tuple):
                ...
                for sType, sName, sVal in zip(struct_name, struct_type, struct_value):
                    struct_values_list.append(
                        {
                            "structure_type": sType + " " + sName,
                            "structure_value": str(sVal),
                        }
                    )
                dictName["parameters"].append(
                    {
                        "type": params_types[t] + " " + params_names[t],
                ...
                type_value = []
                type_value.append(
                    {"type": params_types[t], "value": str(paramValues[t])}
                )
                dictName["parameters"].append(
                    {"type": params_names[t], "value": type_value}
                )
            t += 1
        return dictName
