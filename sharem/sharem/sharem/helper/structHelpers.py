...
HACCEL_32BIT = HBITMAP_32BIT = HBRUSH_32BIT = HCOLORSPACE_32BIT = HDC_32BIT = (
    HDESK_32BIT
) = HDWP_32BIT = HENHMETAFILE_32BIT = HFONT_32BIT = HGDIOBJ_32BIT = HGLOBAL_32BIT = (
    HHOOK_32BIT
) = HICON_32BIT = HINSTANCE_32BIT = HKEY_32BIT = HKL_32BIT = HLOCAL_32BIT = (
    HMENU_32BIT
) = HMETAFILE_32BIT = HMODULE_32BIT = HMONITOR_32BIT = HPALETTE_32BIT = HPEN_32BIT = (
    HRGN_32BIT
) = HRSRC_32BIT = HSTR_32BIT = HTASK_32BIT = HWINSTA_32BIT = HWND_32BIT = (
    SC_HANDLE_32BIT
...
HACCEL_64BIT = HBITMAP_64BIT = HBRUSH_64BIT = HCOLORSPACE_64BIT = HDC_64BIT = (
    HDESK_64BIT
) = HDWP_64BIT = HENHMETAFILE_64BIT = HFONT_64BIT = HGDIOBJ_64BIT = HGLOBAL_64BIT = (
    HHOOK_64BIT
) = HICON_64BIT = HINSTANCE_64BIT = HKEY_64BIT = HKL_64BIT = HLOCAL_64BIT = (
    HMENU_64BIT
) = HMETAFILE_64BIT = HMODULE_64BIT = HMONITOR_64BIT = HPALETTE_64BIT = HPEN_64BIT = (
    HRGN_64BIT
) = HRSRC_64BIT = HSTR_64BIT = HTASK_64BIT = HWINSTA_64BIT = HWND_64BIT = (
    SC_HANDLE_64BIT
) = SERVICE_STATUS_HANDLE_64BIT = HANDLE_64BIT

# Pointer to Types / Handles 32 Bit
LPBOOL_32BIT = PBOOL_32BIT = PBOOLEAN_32BIT = LPBYTE_32BIT = PBYTE_32BIT = (
    PCHAR_32BIT
) = LPCOLORREF_32BIT = LPDWORD_32BIT = PDWORD_32BIT = LPFILETIME_32BIT = (
    PFILETIME_32BIT
) = PFLOAT_32BIT = LPHANDLE_32BIT = PHANDLE_32BIT = PHKEY_32BIT = LPHKL_32BIT = (
    LPINT_32BIT
) = PINT_32BIT = PLCID_32BIT = LPLONG_32BIT = PLONG_32BIT = LPSC_HANDLE_32BIT = (
    PSHORT_32BIT
) = LPUINT_32BIT = PUINT_32BIT = PULONG_32BIT = PUSHORT_32BIT = PWCHAR_32BIT = (
    LPWORD_32BIT
) = PWORD_32BIT = c_uint32

# Pointer to Types / Handles 64 Bit
LPBOOL_64BIT = PBOOL_64BIT = PBOOLEAN_64BIT = LPBYTE_64BIT = PBYTE_64BIT = (
    PCHAR_64BIT
) = LPCOLORREF_64BIT = LPDWORD_64BIT = PDWORD_64BIT = LPFILETIME_64BIT = (
    PFILETIME_64BIT
) = PFLOAT_64BIT = LPHANDLE_64BIT = PHANDLE_64BIT = PHKEY_64BIT = LPHKL_64BIT = (
    LPINT_64BIT
) = PINT_64BIT = PLCID_64BIT = LPLONG_64BIT = PLONG_64BIT = LPSC_HANDLE_64BIT = (
    PSHORT_64BIT
) = LPUINT_64BIT = PUINT_64BIT = PULONG_64BIT = PUSHORT_64BIT = PWCHAR_64BIT = (
    LPWORD_64BIT
) = PWORD_64BIT = c_uint64
...
def makeSubStructVals(uc: Uc, struct):
    ...
    for name in pNames:
        ...
        if "_Array_" in str(value):
            value = value[:]
        elif "emu_helpers.structures" in str(
            value
        ):  # Needed for Additonal Nested Structures
...
def makeSubStructValsString(uc: Uc, struct):
    ...
    for t, n, v in zip(pTypes, pNames, pVals):
        stringForm += (
            f"{constants.GREEN}{t} {n}: {constants.WHITE}{v}{constants.YELLOW}, "
        )
    if stringForm[-1] == " ":
